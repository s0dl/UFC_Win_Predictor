...
FIGHTER_STATISTICS_COLUMN_NAMES = [
    'name',
    'nickname',
    'wins',
    'losses',
...
def clean_missing_value(value: str) -> str:
    '''
    convert UFCStats missing value markers to empty strings
    '''
    if value is None:
        return ''
...
def convert_height_to_cm(height: str) -> str:
    '''
    convert UFCStats height values like 5' 11" to centimeters
    '''
    height = clean_missing_value(height)
    ...
    total_inches = (int(match.group(1)) * 12) + int(match.group(2))
    return round(total_inches * 2.54, 2)


def convert_weight_to_kg(weight: str) -> str:
    '''
    convert UFCStats weight values like 155 lbs. to kilograms
    '''
    weight = clean_missing_value(weight)
...
def convert_reach_to_cm(reach: str) -> str:
    '''
    convert UFCStats reach values like 76" to centimeters
    '''
    reach = clean_missing_value(reach)

    if not reach:
        return ''

    match = re.search(r'(\d+(?:\.\d+)?)', reach)

    if not match:
        return ''

    return round(float(match.group(1)) * 2.54, 2)


def convert_date_of_birth(date_of_birth: str) -> str:
    '''
    convert UFCStats dates like Jul 13, 1978 to YYYY-MM-DD
    '''
    date_of_birth = clean_missing_value(date_of_birth)

    if not date_of_birth:
        return ''

    try:
        return datetime.strptime(date_of_birth, '%b %d, %Y').strftime('%Y-%m-%d')
    except ValueError:
        return ''


def convert_percentage_to_number(percentage: str) -> str:
    '''
    convert UFCStats percentages like 42% to numeric percentages
    '''
    percentage = clean_missing_value(percentage)
...
def convert_number(value: str) -> str:
    '''
    convert UFCStats numeric strings to floats
    '''
    value = clean_missing_value(value)
...
def parse_labeled_fighter_page_values(soup: BeautifulSoup) -> dict:
    '''
    parse label/value pairs from the UFCStats fighter profile page
    '''
    labeled_values = {}
...
def parse_fighter_record(soup: BeautifulSoup) -> Tuple[str, str, str]:
    '''
    parse wins, losses, and draws from the UFCStats fighter profile page
    '''
    ...
    record = re.sub(r'[^0-9-]', '', record_tag.text.strip())
    record_parts = record.split('-')

    if len(record_parts) < 3:
        return '', '', ''

    return record_parts[0], record_parts[1], record_parts[2]


def parse_fighter_statistics(soup: BeautifulSoup, fighter_statistics_column_names: List[str] = FIGHTER_STATISTICS_COLUMN_NAMES) -> pd.DataFrame:
    '''
    parse a UFCStats fighter profile into the ufc-fighters-statistics.csv schema
    '''
    ...
    nickname = nickname_tag.text.strip().replace('"', '') if nickname_tag else ''

    wins, losses, draws = parse_fighter_record(soup)
    labeled_values = parse_labeled_fighter_page_values(soup)
    ...
    fighter_statistics = {
        ...
        'losses': losses,
        'draws': draws,
        'height_cm': convert_height_to_cm(labeled_values.get('Height:', '')),
        'weight_in_kg': convert_weight_to_kg(labeled_values.get('Weight:', '')),
        'reach_in_cm': convert_reach_to_cm(labeled_values.get('Reach:', '')),
        'stance': clean_missing_value(labeled_values.get('STANCE:', '')),
        'date_of_birth': convert_date_of_birth(labeled_values.get('DOB:', '')),
        'significant_strikes_landed_per_minute': convert_number(labeled_values.get('SLpM:', '')),
        'significant_striking_accuracy': convert_percentage_to_number(labeled_values.get('Str. Acc.:', '')),
        'significant_strikes_absorbed_per_minute': convert_number(labeled_values.get('SApM:', '')),
        'significant_strike_defence': convert_percentage_to_number(labeled_values.get('Str. Def:', '')),
        'average_takedowns_landed_per_15_minutes': convert_number(labeled_values.get('TD Avg.:', '')),
        'takedown_accuracy': convert_percentage_to_number(labeled_values.get('TD Acc.:', '')),
        'takedown_defense': convert_percentage_to_number(labeled_values.get('TD Def.:', '')),
        'average_submissions_attempted_per_15_minutes': convert_number(labeled_values.get('Sub. Avg.:', '')),
    }

    return pd.DataFrame([fighter_statistics], columns=fighter_statistics_column_names)
